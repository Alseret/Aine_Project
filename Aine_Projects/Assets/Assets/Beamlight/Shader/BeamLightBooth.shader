Shader "Noriben/BeamLightBooth"
{
	Properties
	{
		_NoiseTex ("NoiseTex", 2D) = "white" {} //ノイズ画像
		_Color ("Color" , Color) = (1.0, 1.0, 1.0, 1.0)
		[Space]
		_TexPower ("TexPower", Range(0, 1)) = 1
		_RimPower ("RimPower", Range(0.01, 10.0)) = 3
		[Space]
		_GradHeight ("GradHeight", Range(-100, 100)) = 1
		_GradPower ("GradPower", Range(1, 0)) = 0.3
		[Space]
		_ConeWidth ("ConeWidth", Range(-0.07, 0.5)) = 1
		[Space]
		_Divide ("Divide", Range(0, 30)) = 0
		_DivideScroll("DivideScroll", Range(-10, 10)) = 0
		_DividePower ("DividePower", Range(0, 1)) = 0
		[HideInInspector]
		_LightIndex ("LightIndex", float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				//normal方向に拡大
				//v.vertex.xyz = v.vertex + v.normal * 0.1 * vertexHeight;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				//ビームの分割
				float pi = 3.14159265;
				float divide = sin(i.uv2.x * pi * floor(_Divide) * 2 + (_Time.w * _DivideScroll)) + 1.0;
				divide = lerp(1.0, divide, _DividePower);
				//合成と明るさ調整・ビームが太くなるほど暗くする
				float baseCol = grad * 20  * mask * rim * divide * pow(1.5, - _ConeWidth * 20);
